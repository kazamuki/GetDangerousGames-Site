...
**Full UX/accessibility review pass (2026-09-22):** Ken asked for an open-ended review of the whole site. Findings were verified with scripted checks across all 23 sitemap pages (accessible names, heading order, iframe titles, broken internal links, overflow at 320/375/768px), not just screenshots. Changes:
- **Mobile header.** Below 720px the sticky header used to wrap into 2–3 rows (146px tall at 375px, 183px at 320px) with 12px nav links. It now stays one 61px row, and the nav collapses behind a menu button (`.nav-toggle` in `main.css`, `assets/js/site-nav.js`; Escape and outside-click close it). The nav is now data-driven from `_data/navigation.yml` (`primary` = header + footer, `secondary` = footer-only Contact/Changelog), so add pages there, not in the layout. The current section gets `.is-active` (a gold underline), plus `aria-current="page"` on exact matches. Writing stays lit on story pages and Blog on posts.
- **Light-mode story header bug.** `.story-header` has no photo of its own, so in light mode its translucent scrim sat over the light page and left the white title and byline on muddy gray. It's now pinned to `var(--midnight)` like the other always-dark bands, with the fade to `--bg-page` held to the bottom 70px (`.page-header.story-header` gets 88px bottom padding so the fade never reaches the text on phones).
- **YouTube cards have real thumbnails.** Playlist data lives in `_data/playlists.yml` and feeds both `/youtube/` (via `_includes/playlist-card.html`) and Home's YouTube spotlight strip, so there's one place to edit. Note the thumbnail `<img>` is absolutely positioned on purpose: a 4:3 `hqdefault` image otherwise stretches the 16:9 `aspect-ratio` box back to 4:3 and the letterbox bars show.
- **Blog cards show post art.** `_includes/post-card.html` is shared by Home and `/blog/`. Posts without their own `image:` inherit `/assets/images/blog-header.jpg` from a new `_config.yml` posts default, which also gives every post an `og:image`. The card swaps that fallback for a branded bear-on-gradient tile so a run of old posts doesn't repeat one photo. Home's "Latest from the studio" heading and CTA now sit on the same 820px column as the cards. Posts with `author:` show the avatar byline.
- **Accessibility.** Added a site-wide gold `:focus-visible` ring (there were no focus styles at all before), a skip link, `aria-label`s on the icon-only social links, titles on all 17 Spotify iframes, `aria-pressed` on the blog tag pills, `prefers-reduced-motion` handling (no hover lifts, no spotlight auto-rotation), and fixed h1→h3 skips on Blog/Changelog. Footer social icons are a fixed 36px now, instead of shrinking to 22px on phones via the old header clamp().
- **Smaller fixes.** `jekyll-feed` enabled (`/feed.xml`, already bundled by github-pages). Added a branded `404.html`. The spotlight strips no longer flash on page load and pause while the tab is hidden. The Writing page's "SEVEN STORIES" count is now computed from the collection. The Podcasts page's "Watch the full run on YouTube" link was unstyled (`.media-link` only applied inside `.media-card`) and is fixed. CSS/JS URLs get a `?v=<build time>` cache-buster so visitors don't pair new HTML with stale CSS after a deploy.
- **"Read on Patreon" button.** Any post can set `patreon: <url>` in its front matter, and `_layouts/post.html` renders a `.patreon-cta` block after the body. The Unseen Court and Social Encounters Pt 1 posts have it set. **Use this for every future Patreon-teaser post** rather than relying on the inline link alone.
- **Shadows logo glow (Home).** The logo's near-black wordmark disappeared on its dark tile. `.band-visual-card img` now gets stacked `drop-shadow()` filters, which follow the PNG's alpha shape to rim it in light and magenta, plus a radial glow on the tile. This is CSS only, so no new logo art was needed.
- **Short meta descriptions.** Every story, and any post whose `excerpt` runs past about 155 characters, now has its own short `description:` (≤155 characters, which is what Google shows) for search results and link previews. The card copy is unchanged: stories still use `blurb` and posts use `excerpt`. This replaces the older "description = same text as blurb" convention from the SEO pass above. **For new stories and posts, write both**: the longer hook for the card and a ≤155-character `description` for search.
- **Wishlist.** The review's forward-looking suggestions (read-next links, series nav, email list, click-to-play Spotify embeds, link checking in CI, etc.) are in `content/wishlist.md` under "From the full-site UX review".
- **Dev env on this workstation:** WSL isn't installed here (`C:\Users\kazam\Github\...`, not the `C:\Apps\...` machine above), but Smart App Control is off, so native Windows Ruby at `C:\Ruby40-x64` works. `.claude/launch.json` gained a `jekyll-site-windows` config on port 4001; 4000 was taken by the sibling Shadows site's dev server.
